    ...
    <Link href={`/products/${product.id}`} className="group flex flex-col relative w-full h-full transition-all duration-500 hover:shadow-[0_20px_60px_rgba(0,0,0,0.06)] overflow-hidden bg-white border border-[#EAE3D8]">
      ...
      <div className="aspect-[4/5] bg-[#F9F8F6] relative overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10 items-start">
          ...
          {product.badge && (
            <span className="bg-[#1A1A1A] text-white text-[9px] font-sans font-medium uppercase tracking-[0.2em] px-2.5 py-1.5 shadow-sm">
              {product.badge}
            </span>
          )}
          ...
          {(product.sale || (product.salePrice && product.salePrice < product.price)) && (
            <span className="bg-white/90 text-[#1A1A1A] text-[9px] font-sans font-medium uppercase tracking-[0.2em] px-2.5 py-1.5 shadow-sm">
      ...
      <div className="p-5 flex flex-col flex-grow bg-white relative z-10">
        <h3 className="text-[14px] font-serif text-[#1A1A1A] leading-relaxed mb-2 group-hover:text-[#8C8C8C] transition-colors duration-500 h-[44px] line-clamp-2">
          {product.name}
        </h3>
        ...
        <div className="flex items-center gap-0.5 mb-3">
          {[1,2,3,4,5].map(star => (
            <svg key={star} className={`w-[11px] h-[11px] ${star <= Math.round(product.reviews || 4) ? 'text-[#C5A880]' : 'text-[#F3F0EB]'}`} fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
          ))}
          <span className="text-[10px] text-[#A3A3A3] ml-1.5 tracking-wider">({product.reviewCount || product.reviews || 4})</span>
        </div>

        {/* Pricing */}
        <div className="flex items-center gap-2.5 font-sans mt-auto">
          <span className="text-[#1A1A1A] font-medium text-[13px] tracking-widest">
          ...
          {(product.oldPrice || (product.salePrice && product.salePrice < product.price)) && (
            <span className="text-[#A3A3A3] line-through text-[11px] tracking-widest">
